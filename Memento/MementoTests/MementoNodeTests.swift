...
class MementoNodeTests: XCTestCase {
    ...
    func testPlistEncoding() {
        ...
        XCTAssertEqual(rep[Constants.bgImageKey] as NSString, "A.png")
        XCTAssertEqual((rep.objectForKey(Constants.overlayKey) as NSArray).count, 0)
        XCTAssertEqual((rep.objectForKey(Constants.placeHolderKey) as NSArray).count, 0)
        XCTAssertEqual((rep.objectForKey(Constants.valueKey) as NSArray).count, 0)
        ...
        XCTAssertEqual((rep.objectForKey(Constants.overlayKey) as NSArray).count, 2)
        XCTAssertEqual(MutableOverlay.decodeFromString((rep.objectForKey(Constants.overlayKey) as NSArray).objectAtIndex(0) as String), overlay1)
        XCTAssertEqual(MutableOverlay.decodeFromString((rep.objectForKey(Constants.overlayKey) as NSArray).objectAtIndex(1) as String), overlay2)
        
        //Checking the placeholders' conversion.
        //This is a way to decode the placehodler from the node's plist representation.
        XCTAssertEqual((rep.objectForKey(Constants.placeHolderKey) as NSArray).count, 2)
        XCTAssertEqual(PlaceHolder.decodeFromString((rep.objectForKey(Constants.placeHolderKey) as NSArray).objectAtIndex(0) as String), placeHolder1)
        XCTAssertEqual(PlaceHolder.decodeFromString((rep.objectForKey(Constants.placeHolderKey) as NSArray).objectAtIndex(1) as String), placeHolder2)
        
        //Checking that null values are added corresponding to each placeholder.
        XCTAssertEqual((rep.objectForKey(Constants.valueKey) as NSArray).count, 2)
        XCTAssertEqual((rep.objectForKey(Constants.valueKey) as NSArray).objectAtIndex(0) as String, "")
        XCTAssertEqual((rep.objectForKey(Constants.valueKey) as NSArray).objectAtIndex(1) as String, "")
        ...
        XCTAssertEqual((rep.objectForKey(Constants.valueKey) as NSArray).count, 2)
        XCTAssertEqual((rep.objectForKey(Constants.valueKey) as NSArray).objectAtIndex(0) as String, "")
        XCTAssertEqual((rep.objectForKey(Constants.valueKey) as NSArray).objectAtIndex(1) as String, "Hello")
    }
